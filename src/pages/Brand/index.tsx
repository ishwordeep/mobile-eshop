...
import { useDeleteBrand, useFetchBrand } from "@/services/service-brand";
...
const Brand = () => {
  ...
  useEffect(() => {
    setHeaderData({
      heading: "Brand",
      description: "Welcome to the brand",
    });
  }, [setHeaderData]);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <StatusSwitch id={id as string} isActive={is_active} model="brand" />
        );
      },
    },
    {
  ...
  return (
    <Flex flexDir={"column"} gap={4}>
      ...
      <DataTable
        columns={columns}
        data={brand?.data.rows ?? []}
        isLoading={isLoading || isFetching}
        count={brand?.data.count ?? 0}
        pagination={
          brand?.data.count ?? 0 > 0
            ? {
                manual: true,
                pageCount: brand?.data?.pagination?.last_page ?? 1,
                totalRows: brand?.data?.pagination?.total ?? 0,
                pageParams: {
                  pageIndex,
                  pageSize: 10,
                },
